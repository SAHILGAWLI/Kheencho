...
def enhance_image(image):
    ...
    corrected = cv2.cvtColor(denoised, cv2.COLOR_GRAY2BGR)
    return corrected
...
def extract_document(image):
    ...
    # Get the four corner points of the contour
    rect = cv2.minAreaRect(max_contour)
    box = cv2.boxPoints(rect)
    box = np.int0(box)

    # Apply perspective transformation to extract the document from the image
    width, height = rect[1]
